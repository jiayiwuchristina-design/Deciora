...
def render_job_sidebar() -> bool:
    render_sidebar_track_panel(
        "Career Track",
        "Use Deciora to decide whether a role is worth your application time before you invest further effort.",
        [
            "Pull job text from a URL or paste it manually.",
            "Parse a PDF resume or use the built-in demo profile.",
            "Score fit, flag risk, and recommend whether to apply.",
        ],
    )
    st.sidebar.markdown("---")
    ...
    st.sidebar.caption("LLM configured" if llm_ready else "LLM not configured. Deciora still runs fully in deterministic mode.")
    return use_llm
...
def render_job_page() -> None:
    ...
    render_hero(
        "Career Track",
        "Decide Whether This Role Is Worth Pursuing.",
        "Review fit, risk, recommendation, and evidence support in Deciora's calm three-layer decision flow before you spend more application time.",
    )
    ...
    with right:
        with st.container(border=True):
            st.markdown("### What Deciora Returns")
            st.markdown(
                "\n".join(
                    [
            ...
            mode_label = "Optional LLM refinement available" if use_llm else "Deterministic decision mode active"
            st.markdown(f'<div class="mode-pill">{escape(mode_label)}</div>', unsafe_allow_html=True)
            st.caption(
                "This track preserves the current decision flow: recommendation first, then layered explanation and action support."
            )

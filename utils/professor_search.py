...
PHONE_FIELD_LABELS = ("tel", "telephone", "phone", "mobile", "电话", "办公电话", "联系电话")
ROOM_FIELD_LABELS = ("room", "office", "location", "address", "office location", "办公地点", "办公室", "地址")
PROFILE_REGION_TAGS = ("main", "article", "section", "div", "aside")
...
RESEARCH_MAX_ANALYZED_BLOCKS = _int_env("RESEARCH_MAX_ANALYZED_BLOCKS", 18)
RESEARCH_MAX_LLM_BLOCK_JUDGMENTS = _int_env("RESEARCH_MAX_LLM_BLOCK_JUDGMENTS", 4)
RESEARCH_MAX_RELATED_PAGES = _int_env("RESEARCH_MAX_RELATED_PAGES", 3)
RESEARCH_MAX_ENRICHMENT_PAGES = _int_env("RESEARCH_MAX_ENRICHMENT_PAGES", 3)
RESEARCH_MAX_ENRICHMENT_SEARCH_RESULTS = _int_env("RESEARCH_MAX_ENRICHMENT_SEARCH_RESULTS", 4)
RESEARCH_OVERALL_RUNTIME_BUDGET_SECONDS = _int_env("RESEARCH_OVERALL_RUNTIME_BUDGET_SECONDS", 22)
RESEARCH_MAX_FETCH_ATTEMPTS = _int_env("RESEARCH_MAX_FETCH_ATTEMPTS", 2)
...
def _new_runtime_state() -> dict:
    ...
    return {
        ...
        "deadline": started_at + RESEARCH_OVERALL_RUNTIME_BUDGET_SECONDS,
        "limits_hit": [],
        "judged_block_signatures": set(),
        "seen_block_signatures": set(),
        "llm_block_judgments": 0,
        "analyzed_blocks": 0,
        "fetch_attempts": 0,
        "followed_related_pages": 0,
        "enrichment_pages_checked": 0,
    }


def _record_runtime_limit(runtime_state: dict | None, limit_name: str) -> None:
    if runtime_state is None:
        return
    if limit_name not in runtime_state["limits_hit"]:
        runtime_state["limits_hit"].append(limit_name)


def _runtime_budget_available(runtime_state: dict | None) -> bool:
    if runtime_state is None:
        return True
    available = time.monotonic() < runtime_state["deadline"]
    if not available:
        _record_runtime_limit(runtime_state, "overall_runtime_budget")
    return available


def _block_signature(text: str) -> str:
    normalized = clean_text(text).lower()
    return normalized[:420]


def _consume_block_slot(runtime_state: dict | None, block_text: str) -> bool:
    if runtime_state is None:
        return True
    signature = _block_signature(block_text)
    if signature in runtime_state["seen_block_signatures"]:
        _record_runtime_limit(runtime_state, "duplicate_block_skipped")
        return False
    if runtime_state["analyzed_blocks"] >= RESEARCH_MAX_ANALYZED_BLOCKS:
        _record_runtime_limit(runtime_state, "analyzed_block_cap")
        return False
    runtime_state["seen_block_signatures"].add(signature)
    runtime_state["analyzed_blocks"] += 1
    return True
...
def _extract_labeled_field_value(lines: list[str], labels: tuple[str, ...]) -> str:
    for index, line in enumerate(lines[:24]):
        cleaned = clean_text(line)
        lowered = cleaned.lower()
        if not cleaned:
            continue
        label_present = any(label in lowered for label in labels if label.isascii()) or any(label in cleaned for label in labels if not label.isascii())
        if not label_present:
            continue
        value = cleaned
        for label in labels:
            if label.isascii():
                value = re.sub(rf".*?\b{re.escape(label)}\b[:：]?\s*", "", value, flags=re.IGNORECASE)
            else:
                value = re.sub(rf".*?{re.escape(label)}[:：]?\s*", "", value)
        value = clean_text(value)
        if value and value != cleaned:
            return value
        if index + 1 < len(lines):
            next_line = clean_text(lines[index + 1])
            if next_line and not _looks_like_field_label(next_line):
                return next_line
    return ""


def _extract_profile_contact_fields(
    node: BeautifulSoup,
    page_domain: str,
    *,
    name_hint: str = "",
    ...
    visible_emails = _extract_emails_from_node(node, page_domain, allow_any_visible=True) or _regex_detect_emails(text)
    labeled_email = _regex_detect_emails(_extract_labeled_field_value(lines, PROFILE_CONTACT_LABELS))
    phone = _extract_labeled_field_value(lines, PHONE_FIELD_LABELS)
    room = _extract_labeled_field_value(lines, ROOM_FIELD_LABELS)
    email = ""
    email_source = ""
    normalized_name = _normalize_person_name(name_hint)

    if labeled_email:
        email = labeled_email[0]
        email_source = "Labeled email field on the provided page"
    elif normalized_name:
        email, email_source = _best_email_for_name(visible_emails, normalized_name)
    if not email and single_profile and len(visible_emails) == 1:
        email = visible_emails[0]
        email_source = "Single-profile visible email on the provided page"
    if not email and len(visible_emails) == 1:
        email = visible_emails[0]
        email_source = "Only visible email found in the profile region"
...
def _person_like_name_count(node: BeautifulSoup) -> int:
    count = 0
    seen: set[str] = set()
    for tag in node.find_all(["a", "h1", "h2", "h3", "h4", "strong", "b", "dt", "th", "td", "span"], limit=20):
        candidate = _normalize_person_name(tag.get_text(" ", strip=True))
        if candidate and candidate not in seen and _looks_like_person_name(candidate) and not _looks_like_container_label(candidate):
            seen.add(candidate)
            count += 1
        if count >= 4:
            break
...
def _score_profile_region(node: BeautifulSoup, page_domain: str, name_hint: str) -> int:
    text = clean_text(node.get_text("\n", strip=True))
    word_count = len(text.split())
    if word_count < 4 or word_count > 320:
        return -999
    visible_name = _candidate_display_name(node)
    contact_fields = _extract_profile_contact_fields(node, page_domain, name_hint=name_hint or visible_name, single_profile=True)
    score = 0
    if 10 <= word_count <= 160:
        score += 7
    elif word_count <= 240:
        score += 4
    if visible_name and _looks_like_person_name(visible_name):
        score += 9
    if node.find(re.compile(r"^h[1-2]$")):
        score += 4
    if contact_fields.get("email"):
        score += 8
    elif contact_fields.get("visible_emails"):
        score += 5
    if any(label in text.lower() for label in ("email", "e-mail", "contact")) or any(label in text for label in ("邮箱", "电子邮箱", "联系方式")):
        score += 4
    if _guess_role_title(text[:600], visible_name or name_hint):
        score += 4
    if contact_fields.get("phone"):
        score += 2
    if contact_fields.get("room"):
        score += 2
    person_count = _person_like_name_count(node)
    if person_count > 1:
        score -= min(10, (person_count - 1) * 4)
    return score


def _find_primary_profile_region(soup: BeautifulSoup, page_context: dict) -> BeautifulSoup:
    root = soup.find("main") or soup.find("article") or soup.body or soup
    page_domain = canonical_domain(page_context.get("primary_source_url") or page_context.get("official_profile_url") or "")
    name_hint = _normalize_person_name(page_context.get("lead_contact_name") or page_context.get("name", ""))
    candidates: list[BeautifulSoup] = [root]
    for tag in root.find_all(PROFILE_REGION_TAGS, limit=80):
        text = clean_text(tag.get_text("\n", strip=True))
        if len(text.split()) < 4:
            continue
        if (
            _candidate_display_name(tag)
            or tag.find(re.compile(r"^h[1-2]$"))
            or any(label in text.lower() for label in ("email", "e-mail", "contact"))
            or any(label in text for label in ("邮箱", "电子邮箱", "联系方式"))
        ):
            candidates.append(tag)
    best = root
    best_score = -999
    for candidate in candidates[:40]:
        score = _score_profile_region(candidate, page_domain, name_hint)
        if score > best_score:
            best = candidate
            best_score = score
    return best
...
def _looks_like_person_profile_block(block_text: str, name_hint: str = "", local_emails: list[str] | None = None) -> bool:
    words = len(clean_text(block_text).split())
    if words < 4 or words > 240:
        return False
    normalized_name = _normalize_person_name(name_hint)
    if normalized_name and _looks_like_person_name(normalized_name):
        return True
    lines = [clean_text(line) for line in re.split(r"[\n\r]+", block_text) if clean_text(line)]
    visible_name_count = sum(
        1
        for line in lines[:6]
        if _looks_like_person_name(_normalize_person_name(line)) and not _looks_like_container_label(line)
    )
    has_role = bool(_guess_role_title(block_text[:420], normalized_name))
    return bool(visible_name_count and (has_role or local_emails))
...
def _new_parser_report(target_url: str, page_context: dict, page_text: str) -> dict:
    return {
        ...
        "runtime_limits_hit": [],
        "llm_block_judgments_used": 0,
        "overall_runtime_budget_seconds": RESEARCH_OVERALL_RUNTIME_BUDGET_SECONDS,
        "failure_categories": [],
        "warning_categories": [],
        "events": [],
    }
...
def _selected_binding_issues(candidate: dict) -> list[str]:
    ...
    email_source = clean_text(candidate.get("email_source", "")).lower()
    role_title = clean_text(candidate.get("role_title", ""))
    profile_bound_email = candidate.get("page_type") == "faculty_profile" or any(
        marker in email_source
        for marker in (
            "labeled email field",
            "single-profile visible email",
            "only visible email found in the profile region",
        )
    )
    if not _looks_like_person_name(name):
        issues.append("fallback_to_generic_target")
    if email and name and not _contains_cjk(name) and _email_name_score(email, name) <= 0 and not profile_bound_email:
...
def _should_use_ai_block_judge(
    *,
    block_text: str,
    heuristic_judgment: dict,
    local_emails: list[str],
    ...
    if not is_llm_configured() or not _runtime_budget_available(runtime_state):
        return False
    if runtime_state is not None and runtime_state["llm_block_judgments"] >= RESEARCH_MAX_LLM_BLOCK_JUDGMENTS:
        _record_runtime_limit(runtime_state, "llm_block_judgment_cap")
        return False
    if not _looks_like_person_profile_block(block_text, heuristic_judgment.get("person_name", ""), local_emails):
        return False
    confidence = int(heuristic_judgment.get("confidence", 0))
    has_name = bool(heuristic_judgment.get("person_name"))
    has_title = bool(heuristic_judgment.get("title"))
    has_email = bool(heuristic_judgment.get("email") or local_emails)
    has_topics = bool(heuristic_judgment.get("research_areas") or heuristic_judgment.get("research_interests"))
    if heuristic_judgment.get("is_human_candidate") and confidence >= 74 and (has_email or has_title or has_topics):
        return False
    if not heuristic_judgment.get("is_human_candidate") and confidence <= 32 and not (has_name and (has_title or has_email)):
        return False
    if runtime_state is not None:
        signature = _block_signature(block_text)
        if signature in runtime_state["judged_block_signatures"]:
            _record_runtime_limit(runtime_state, "duplicate_ai_judge_skipped")
            return False
    return True
...
def _judge_research_block(
    *,
    ...
    if _should_use_ai_block_judge(
        ...
        if runtime_state is not None:
            runtime_state["llm_block_judgments"] += 1
            runtime_state["judged_block_signatures"].add(_block_signature(block_text))
...
def _find_mailto_email(soup: BeautifulSoup, page_domain: str) -> tuple[str, str]:
    ...
    contact_fields = _extract_profile_contact_fields(soup, source_domain, single_profile=True)
    email = clean_text(str(contact_fields.get("email", "")))
    if email:
        return email, clean_text(str(contact_fields.get("email_source", ""))) or "Visible email on the provided page"
    emails = [item for item in contact_fields.get("visible_emails", []) if clean_text(str(item))]
    if not emails:
        return "", ""
...
def _fetch_page_html(url: str, runtime_state: dict | None = None) -> tuple[str, str]:
    last_error = ""
    attempts = 0
    while attempts < RESEARCH_MAX_FETCH_ATTEMPTS and _runtime_budget_available(runtime_state):
        attempts += 1
        if runtime_state is not None:
            runtime_state["fetch_attempts"] += 1
        html_text, error = fetch_page_best_effort(url)
        if html_text:
            return html_text, ""
        last_error = error
    if attempts >= RESEARCH_MAX_FETCH_ATTEMPTS:
        _record_runtime_limit(runtime_state, "fetch_retry_cap")
    return "", last_error
...
def _build_candidate_from_block(
    block: BeautifulSoup,
    base_url: str,
    page_context: dict,
    forced_name: str = "",
    ...
    if not _looks_like_person_profile_block(block_text, forced_name or _candidate_display_name(block)):
        return None
    contact_fields = _extract_profile_contact_fields(
        block,
        page_domain,
        name_hint=_normalize_person_name(forced_name) or _candidate_display_name(block),
    )
    emails = list(contact_fields.get("visible_emails", [])) or _regex_detect_emails(block_text)
    ...
    email = judgment.get("email", "") or clean_text(str(contact_fields.get("email", "")))
    email_source = judgment.get("email_source", "") or clean_text(str(contact_fields.get("email_source", "")))
    ...
    candidate = {
        ...
        "phone_number": clean_text(str(contact_fields.get("phone", ""))),
        "office_location": clean_text(str(contact_fields.get("room", ""))),
...
def _extract_text_profile_candidates(page_text: str, base_url: str, page_context: dict, runtime_state: dict | None = None) -> list[dict]:
    ...
    for idx, start in enumerate(name_positions[:20]):
        ...
        if len(clean_text(block_text).split()) < 4:
            continue
        if not _consume_block_slot(runtime_state, block_text):
            continue
        emails = _regex_detect_emails(block_text)
        if not _looks_like_person_profile_block(block_text, name, emails):
            continue
        judgment = _judge_research_block(
            block_text=block_text,
            page_context=page_context,
...
def _extract_multi_profile_candidates(
    soup: BeautifulSoup,
    base_url: str,
    page_context: dict,
    runtime_state: dict | None = None,
    ...
    for node in candidate_nodes[:80]:
        if not _runtime_budget_available(runtime_state):
            break
        ...
        for contact_name in contact_names:
            if contact_name == visible_text or contact_name in visible_text:
                ...
                break
        candidate = _build_candidate_from_block(container, base_url, page_context, forced_name=forced_name, runtime_state=runtime_state)
        if not candidate:
            continue
        if not _is_person_candidate(candidate):
            continue
...
def _extract_primary_profile_candidate(
    soup: BeautifulSoup,
    base_url: str,
    page_context: dict,
    runtime_state: dict | None = None,
) -> dict | None:
    primary_node = _find_primary_profile_region(soup, page_context)
    ...
    local_emails = list(contact_fields.get("visible_emails", [])) or _regex_detect_emails(block_text)
    ...
    if not judgment.get("is_human_candidate"):
        fallback_role = _guess_role_title(block_text, name_hint)
        fallback_email = clean_text(str(contact_fields.get("email", "")))
        if _looks_like_person_name(name_hint) and (fallback_role or fallback_email):
            judgment["person_name"] = name_hint
            judgment["title"] = judgment.get("title", "") or fallback_role
            judgment["email"] = judgment.get("email", "") or fallback_email
            judgment["email_source"] = judgment.get("email_source", "") or clean_text(str(contact_fields.get("email_source", "")))
            judgment["email_confidence"] = _email_confidence(name_hint, judgment.get("email", ""), local_emails, block_text)
            judgment["block_type"] = PERSON_BLOCK_TYPE
            judgment["is_human_candidate"] = True
            judgment["confidence"] = max(int(judgment.get("confidence", 0)), 76)
        else:
            return None
    ...
    candidate = {
        ...
        "department_page_url": page_context.get("department_page_url", ""),
        "opportunity_page_url": page_context.get("opportunity_page_url", ""),
        "official_email": judgment.get("email", "") or clean_text(str(contact_fields.get("email", ""))),
        "email_source": judgment.get("email_source", "") or clean_text(str(contact_fields.get("email_source", ""))),
...
def _parse_research_page(
    ...
    *,
    ...
    if not _runtime_budget_available(runtime_state):
        return None
    html_text, _error = _fetch_page_html(url, runtime_state)
    if not html_text:
        return None
    ...
    profile_region = _find_primary_profile_region(
        soup,
        {
            "primary_source_url": url,
            "official_profile_url": url,
            "name": lead_contact or name or page_heading,
            "lead_contact_name": lead_contact,
        },
    )
    profile_region_text = clean_text(profile_region.get_text("\n", strip=True))
    single_profile_page = page_type not in {"directory_page", "lab_page", "department_page", "center_page"} or len(contacts) <= 1
    contact_fields = _extract_profile_contact_fields(
        profile_region,
        canonical_domain(url),
        name_hint=lead_contact or name or page_heading,
        single_profile=single_profile_page,
    )
    role_title = _guess_role_title(profile_region_text[:900] or text[:1600], lead_contact or name)
    university = _guess_university(url, page_title or result.get("title", ""))
    department = _guess_department(sections, text, page_heading)
    email = clean_text(str(contact_fields.get("email", "")))
    email_source = clean_text(str(contact_fields.get("email_source", "")))
    if not email:
    ...
    research_text = clean_text("\n".join(research_focus) or (profile_region_text[:1800] if single_profile_page else text[:2600]))
    ...
    candidate = {
        ...
        "raw_text_excerpt": (profile_region_text or text)[:2600],
        "phone_number": clean_text(str(contact_fields.get("phone", ""))),
        "office_location": clean_text(str(contact_fields.get("room", ""))),
...
def _merge_target_fields(base: dict, related: dict) -> dict:
    ...
    merged["phone_number"] = related.get("phone_number") or merged.get("phone_number", "")
    merged["office_location"] = related.get("office_location") or merged.get("office_location", "")
...
def _build_page_context_from_html(url: str, soup: BeautifulSoup) -> dict:
    ...
    source_credibility_label, source_credibility_detail = _source_credibility(url, page_type, text, page_title)
    role_title = _guess_role_title(profile_region_text[:900] or text[:1400], lead_contact or name or page_heading)
    context = {
        ...
        "profile_text": (profile_region_text[:1800] if single_profile_page else text[:2600]),
        "bio_or_summary": " ".join(split_sentences((profile_region_text or text)[:1200])[:2])[:320],
        "raw_text_excerpt": (profile_region_text or text)[:2600],
        "phone_number": clean_text(str(contact_fields.get("phone", ""))),
        "office_location": clean_text(str(contact_fields.get("room", ""))),
        "related_candidate_links": _extract_related_candidate_links(soup, url, page_type),
...
def _enrich_url_candidate(
    ...
    if enriched.get("official_email") and (
        enriched.get("page_type") in {"faculty_profile", "other"}
        or enriched.get("profile_signal_count", 0) >= 3
    ):
        return enriched, checked, used_urls
    ...
    base_domain = canonical_domain(base_url)
    if (not urls_to_try or not enriched.get("official_email")) and clean_text(candidate.get("name", "")) and _runtime_budget_available(runtime_state):
        query = clean_text(f'"{candidate.get("name", "")}" "{university_hint or candidate.get("university", "")}" faculty research')
        for result in search_duckduckgo_html(query, max_results=RESEARCH_MAX_ENRICHMENT_SEARCH_RESULTS):
            ...
            if len(urls_to_try) >= RESEARCH_MAX_ENRICHMENT_PAGES:
                break

    for url in urls_to_try[:RESEARCH_MAX_ENRICHMENT_PAGES]:
        if not _runtime_budget_available(runtime_state):
            break
        if checked >= RESEARCH_MAX_ENRICHMENT_PAGES:
            _record_runtime_limit(runtime_state, "enrichment_page_cap")
            break
        checked += 1
        if runtime_state is not None:
            runtime_state["enrichment_pages_checked"] += 1
        parsed = _parse_research_page(
            {
                "url": url,
        ...
        if enriched.get("official_email") and (
            enriched.get("publications") or enriched.get("recent_topics") or enriched.get("profile_signal_count", 0) >= 3
        ):
            break
...
def parse_research_target_url(target_url: str, runtime_state: dict | None = None) -> dict | None:
    normalized_url = _normalize_target_url(target_url)
    if not normalized_url:
        return None

    runtime_state = runtime_state or _new_runtime_state()
    html_text, _error = _fetch_page_html(normalized_url, runtime_state)
    if not html_text:
        return None
    ...
    base_result = _parse_research_page(
        {"url": normalized_url, "title": "", "snippet": "", "source_type": ""},
        allow_non_official=True,
        min_words=20,
        runtime_state=runtime_state,
    )
    ...
    primary_profile_candidate = _extract_primary_profile_candidate(soup, normalized_url, page_context, runtime_state)
    dom_candidates = _extract_multi_profile_candidates(soup, normalized_url, page_context, runtime_state)
    text_candidates = _extract_text_profile_candidates(page_text, normalized_url, page_context, runtime_state)
    ...
    if multi_profile_detected:
        parser_report["runtime_limits_hit"] = runtime_state["limits_hit"]
        parser_report["llm_block_judgments_used"] = runtime_state["llm_block_judgments"]
        return {
    ...
    compatible_single_profile_candidates = []
    for candidate in ([primary_profile_candidate] if primary_profile_candidate else []) + multi_candidates:
        if not candidate:
            continue
        if not _is_person_candidate(candidate):
            continue
        if _is_person_candidate(base_result):
            if not _names_compatible(base_result.get("lead_contact_name") or base_result.get("name", ""), candidate.get("name", "")):
                continue
        compatible_single_profile_candidates.append(candidate)
    compatible_single_profile_candidates.sort(
        key=lambda candidate: (
            -int(bool(candidate.get("official_email"))),
            -int(candidate.get("page_type") == "faculty_profile"),
            -candidate.get("extraction_confidence", 0),
            -candidate.get("profile_signal_count", 0),
        )
    )
    for candidate in compatible_single_profile_candidates:
        merged_candidate = _merge_target_fields(base_result, candidate)
        if (
            not _is_person_candidate(base_result)
            or (candidate.get("official_email") and not base_result.get("official_email"))
            or candidate.get("extraction_confidence", 0) > base_result.get("extraction_confidence", 0)
            or candidate.get("page_type") == "faculty_profile"
        ):
            base_result = merged_candidate
            parser_report["fallback_path_used"] = parser_report["fallback_path_used"] or "single_profile_candidate_merge"
    ...
    if soup is not None and not base_result.get("official_email") and _runtime_budget_available(runtime_state):
        for related_page in _collect_related_pages_for_target(soup, normalized_url)[:RESEARCH_MAX_RELATED_PAGES]:
            if not _runtime_budget_available(runtime_state):
                break
            if related_pages_checked >= RESEARCH_MAX_RELATED_PAGES:
                _record_runtime_limit(runtime_state, "related_page_cap")
                break
            related_pages_checked += 1
            runtime_state["followed_related_pages"] += 1
...
def analyze_research_target(
    *,
    ...
    target_parse = parse_research_target_url(target_url, runtime_state)
    errors: list[str] = []
    if not target_parse:
    ...
    for candidate in raw_candidates:
        enriched_candidate, checked, used = _enrich_url_candidate(
            _enforce_candidate_consistency(candidate),
            normalized_target,
            page_context.get("university", ""),
            runtime_state,
        )

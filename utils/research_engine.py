...
from utils.llm import call_research_email_polish, is_llm_configured, research_polish_model_name, research_standard_model_name
...
RESEARCH_STATE_DEFAULTS = {
    ...
    "research_polished_packages": {},
    "research_polish_errors": {},
}
...
def load_research_demo_inputs() -> None:
    ...
    st.session_state["research_polished_packages"] = {}
    st.session_state["research_polish_errors"] = {}
...
def render_research_sidebar() -> None:
    render_sidebar_track_panel(
        "Research Track",
        "Use Deciora to judge whether a research target is worth contacting and to keep outreach grounded in source evidence.",
        [
            "Parse a CV and extract research-relevant signals.",
            "Parse the professor, lab, or research-group URL you provide.",
            "Decide whether the target is worth contacting and prepare grounded outreach support.",
        ],
    )
    st.sidebar.markdown("---")
...
def _polish_package_key(selected: dict[str, Any]) -> str:
    return selected.get("candidate_id") or selected.get("official_profile_url") or selected.get("primary_source_url") or selected.get("name", "")


def _build_email_polish_payload(selected: dict[str, Any], package: dict[str, Any]) -> dict[str, Any]:
    return {
        "selected_candidate": {
            "name": selected.get("name", ""),
            "role_title": selected.get("role_title", ""),
            "university": selected.get("university", ""),
            "department_or_lab": selected.get("department_or_lab", ""),
            "official_email": selected.get("official_email", ""),
            "primary_source_url": selected.get("primary_source_url") or selected.get("official_profile_url", ""),
            "research_tags": selected.get("research_tags", [])[:5],
            "recent_topics": selected.get("recent_topics", [])[:3],
            "publications": selected.get("publications", [])[:2],
            "why_match": selected.get("why_match", [])[:3],
            "contact_feasibility": selected.get("contact_feasibility", [])[:2],
        },
        "draft_package": {
            "subject_lines": package.get("subject_lines", [])[:3],
            "personalization_lines": package.get("personalization_lines", [])[:3],
            "email_draft": package.get("email_draft", ""),
            "greeting": package.get("greeting", ""),
            "recipient_email": package.get("recipient_email", ""),
        },
        "instruction": "Polish the draft while staying fully faithful to the provided evidence and recipient details.",
    }


def _get_display_outreach_package(selected: dict[str, Any], package: dict[str, Any]) -> dict[str, Any]:
    polished_packages = st.session_state.get("research_polished_packages", {})
    stored = polished_packages.get(_polish_package_key(selected))
    if not stored:
        return package
    return {
        **package,
        **stored,
        "recipient_email": package.get("recipient_email", ""),
        "selected_candidate_id": package.get("selected_candidate_id", ""),
        "selected_candidate_name": package.get("selected_candidate_name", ""),
        "selected_candidate_title": package.get("selected_candidate_title", ""),
    }


def _polish_outreach_package(selected: dict[str, Any], package: dict[str, Any]) -> tuple[dict[str, Any] | None, str]:
    payload = _build_email_polish_payload(selected, package)
    polished, error = call_research_email_polish(payload)
    if not polished:
        return None, error
    subject_lines = [clean_text(polished.get("subject_line", ""))] + [item for item in package.get("subject_lines", []) if clean_text(item)]
    ...
    for line in subject_lines:
        ...
        if not cleaned or lowered in seen:
            continue
        deduped_subjects.append(cleaned)
        seen.add(lowered)
        if len(deduped_subjects) >= 3:
            break
    return {
        **package,
        "subject_lines": deduped_subjects or package.get("subject_lines", []),
        "email_draft": polished.get("email_draft", package.get("email_draft", "")),
        "polished_with_model": research_polish_model_name(),
    }, ""


def render_email_actions(selected: dict[str, Any], package: dict[str, Any]) -> dict[str, Any]:
    active_package = _get_display_outreach_package(selected, package)
    package_key = _polish_package_key(selected)
    polish_errors = st.session_state.get("research_polish_errors", {})
    with st.container(border=True):
        st.markdown("### Email Draft Actions")
        subject_line = (active_package.get("subject_lines") or ["Research outreach"])[0]
        draft_body = active_package.get("email_draft", "")
        if is_llm_configured():
            if st.button("Polish outreach email", use_container_width=True, key=f"research_polish_email_{abs(hash(package_key)) % 1_000_000}"):
                with st.spinner("Polishing the outreach email with the stronger model..."):
                    polished_package, error = _polish_outreach_package(selected, active_package)
                if polished_package:
                    st.session_state["research_polished_packages"] = {
                        **st.session_state.get("research_polished_packages", {}),
                        package_key: polished_package,
                    }
                    if package_key in polish_errors:
                        updated_errors = {**polish_errors}
                        updated_errors.pop(package_key, None)
                        st.session_state["research_polish_errors"] = updated_errors
                        polish_errors = updated_errors
                    active_package = polished_package
                    subject_line = (active_package.get("subject_lines") or ["Research outreach"])[0]
                    draft_body = active_package.get("email_draft", "")
                    st.success(f"Email polished with {research_polish_model_name()}.")
                else:
                    st.session_state["research_polish_errors"] = {
                        **polish_errors,
                        package_key: error or "Polish failed.",
                    }
                    polish_errors = st.session_state["research_polish_errors"]
            st.caption(
                f'When LLM assistance is needed, default Research Track analysis uses {research_standard_model_name()}. '
                'The stronger model is used only when you click "Polish outreach email".'
            )
        if polish_errors.get(package_key):
            st.warning(polish_errors[package_key])
...
def render_selected_match(selected: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    active_package = _get_display_outreach_package(selected, layer3["outreach_package"])
    layer3["outreach_package"] = active_package
    ...
    layer3["outreach_package"] = render_email_actions(selected, layer3["outreach_package"])
...
def render_research_page() -> None:
    ...
    render_hero(
        "Research Track",
        "Decide Whether This Target Is Worth Contacting.",
        "Bring a professor, lab, faculty, or research-group URL plus your CV, then let Deciora evaluate fit, outreach feasibility, and source support before you send anything.",
    )
    ...
    with right:
        with st.container(border=True):
            ...
            st.markdown('<div class="mode-pill">Decision lens for a provided research URL</div>', unsafe_allow_html=True)
